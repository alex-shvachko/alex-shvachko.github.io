...
const pointer = { x: 0, y: 0 }, cameraControl = { theta: 0, phi: .08, radius: 10.5, dragging: false, x: 0, y: 0 }, fallbackView = { x: 50, y: 50, zoom: 100 };
const updateFallbackView = () => { canvas.parentElement.style.backgroundPosition = `${fallbackView.x}% ${fallbackView.y}%`; canvas.parentElement.style.backgroundSize = `${fallbackView.zoom}% auto`; };
addEventListener('pointermove', event => { pointer.x = event.clientX / innerWidth - .5; pointer.y = event.clientY / innerHeight - .5; if (cameraControl.dragging) { const dx = event.clientX - cameraControl.x, dy = event.clientY - cameraControl.y; cameraControl.theta -= dx * .006; cameraControl.phi = THREE.MathUtils.clamp(cameraControl.phi + dy * .004, -.65, .8); fallbackView.x = THREE.MathUtils.clamp(fallbackView.x - dx * .08, 0, 100); fallbackView.y = THREE.MathUtils.clamp(fallbackView.y - dy * .08, 0, 100); updateFallbackView(); cameraControl.x = event.clientX; cameraControl.y = event.clientY; } });
...
addEventListener('wheel', event => { event.preventDefault(); cameraControl.radius = THREE.MathUtils.clamp(cameraControl.radius + event.deltaY * .012, 6.2, 16); fallbackView.zoom = THREE.MathUtils.clamp(fallbackView.zoom - event.deltaY * .035, 85, 180); updateFallbackView(); }, { passive: false });
