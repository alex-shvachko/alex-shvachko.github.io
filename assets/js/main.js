...
interactionTarget.addEventListener('wheel', event => { event.preventDefault(); cameraControl.radius = THREE.MathUtils.clamp(cameraControl.radius + event.deltaY * .012, 6.2, 16); fallbackView.zoom = THREE.MathUtils.clamp(fallbackView.zoom - event.deltaY * .035, 85, 180); updateFallbackView(); }, { passive: false });
